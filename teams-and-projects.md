...
Критерии оценки:
- [ ] В Github Actions успешно настроена **компиляция проекта и запуск тестов** на каждый коммит в ветке `main` и в пул-реквестах.

🥇 Бонусы:
- [ ] `+0.05`: Настроена сборка проекта из ветки `main` в исполняемый артефакт (докер-образ, native-image, jlink/jpackage).
